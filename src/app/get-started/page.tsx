...
export default function GetStarted() {
  ...
  useEffect(() => {
    ...
    // If already signed in, skip signup entirely
  ...
  const C = {
    ...
    green:'#5DCC8A', greenDim:'rgba(93,204,138,0.08)', red:'#E05252',
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .progress-bar { position: fixed; top: 64px; left: 0; right: 0; z-index: 99; height: 2px; background: ${C.border}; }
        .progress-fill { height: 100%; background: linear-gradient(90deg, ${C.indigo}, ${C.indigoL}); transition: width 0.5s cubic-bezier(0.16,1,0.3,1); border-radius: 0 2px 2px 0; }
        .center { min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 90px 20px 60px; position: relative; z-index: 1; }
        .panel { width: 100%; max-width: 460px; opacity: 0; transform: translateY(16px); transition: opacity 0.5s ease, transform 0.5s ease; }
        .panel.visible { opacity: 1; transform: translateY(0); }
        .eyebrow { font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 0.2em; text-transform: uppercase; color: ${C.indigoL}; display: flex; align-items: center; gap: 10px; margin-bottom: 16px; }
        .eyebrow-line { width: 20px; height: 1px; background: ${C.indigoL}; }
        .panel-title { font-family: 'Syne', sans-serif; font-size: 28px; font-weight: 800; color: ${C.textP}; letter-spacing: -0.8px; line-height: 1.05; margin-bottom: 8px; }
        .panel-sub { font-family: 'DM Sans', sans-serif; font-size: 14px; color: ${C.textS}; line-height: 1.6; margin-bottom: 28px; }
        ...
        .type-card { padding: 18px 20px; border: 0.5px solid ${C.borderMd}; border-radius: 14px; background: ${C.surface2}; cursor: pointer; transition: all 0.15s; display: flex; align-items: center; gap: 16px; text-align: left; width: 100%; }
        .type-card:hover { border-color: rgba(91,79,232,0.3); background: ${C.surface3}; }
        .type-card.selected { border-color: rgba(91,79,232,0.5); background: ${C.indigoDim}; }
        .type-icon { width: 44px; height: 44px; border-radius: 12px; background: ${C.surface3}; border: 0.5px solid ${C.border}; display: flex; align-items: center; justify-content: center; color: ${C.textS}; flex-shrink: 0; transition: all 0.15s; }
        .type-card.selected .type-icon { background: ${C.indigoDim}; border-color: rgba(91,79,232,0.3); color: ${C.indigoL}; }
        .type-body { flex: 1; min-width: 0; }
        .type-title { font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 800; color: ${C.textP}; letter-spacing: -0.2px; margin-bottom: 3px; }
        .type-desc { font-family: 'DM Sans', sans-serif; font-size: 12px; color: ${C.textS}; line-height: 1.5; }
        .type-badge { font-family: 'DM Mono', monospace; font-size: 9px; padding: 3px 9px; border-radius: 5px; flex-shrink: 0; color: ${C.indigoL}; background: ${C.indigoDim}; border: 0.5px solid rgba(91,79,232,0.2); }
        ...
        .btn-primary { width: 100%; padding: 14px; border-radius: 12px; background: ${C.indigo}; color: #fff; border: none; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; transition: background 0.15s; box-shadow: 0 4px 20px rgba(91,79,232,0.3); }
        .btn-primary:hover { background: ${C.indigoL}; }
        .btn-primary:disabled { opacity: 0.7; cursor: wait; }
        .btn-continue { width: 100%; padding: 14px; border-radius: 12px; background: ${C.indigo}; color: #fff; border: none; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.15s; box-shadow: 0 4px 20px rgba(91,79,232,0.3); margin-top: 20px; display: flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        .done-box { text-align: center; padding: 40px 28px; display: flex; flex-direction: column; align-items: center; gap: 16px; }
        .done-icon { width: 56px; height: 56px; border-radius: 50%; background: ${C.greenDim}; border: 0.5px solid rgba(93,204,138,0.25); display: flex; align-items: center; justify-content: center; color: ${C.green}; }
        .done-title { font-family: 'Syne', sans-serif; font-size: 22px; font-weight: 800; color: ${C.textP}; letter-spacing: -0.5px; }
        .done-desc { font-family: 'DM Sans', sans-serif; font-size: 13px; color: ${C.textS}; line-height: 1.6; max-width: 300px; }
        .btn-dashboard { padding: 12px 28px; border-radius: 12px; background: ${C.indigo}; color: #fff; border: none; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500; cursor: pointer; transition: background 0.15s; text-decoration: none; display: inline-block; }
      ...
      <div className="progress-bar" aria-hidden="true">
        <div className="progress-fill" style={{ width: step === 'type' ? '33%' : step === 'details' ? '66%' : '100%' }} />
      </div>
      ...
      <nav className="topbar">
        ...
        <a href={redirectTo ? `/sign-in?redirectTo=${encodeURIComponent(redirectTo)}` : '/sign-in'} className="nav-link">Sign in</a>
      </nav>
          ...
          {step === 'type' && (
            <>
              ...
              <div className="type-grid">
                ...
                <button className={`type-card tb ${accountType === 'business' ? 'selected' : ''}`} onClick={() => setAccountType('business')}>
              ...
              <button className="btn-continue tb" disabled={!accountType} onClick={() => setStep('details')}>
                Continue
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
              </button>
              <div className="panel-foot">Already have an account? <a href={redirectTo ? `/sign-in?redirectTo=${encodeURIComponent(redirectTo)}` : '/sign-in'}>Sign in</a></div>
            </>
          )}

          {step === 'details' && (
            <>
              <div className="eyebrow"><span className="eyebrow-line" />Step 2 of 2 · {accountType === 'individual' ? 'Individual account' : 'Business account'}</div>
              ...
              <div className="card">
                ...
                <div className="fields">
                  <input className="field-input" type="text" placeholder={accountType === 'individual' ? 'Full name' : 'Company name'} value={name} onChange={e => setName(e.target.value)} />
                  <input className="field-input" type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSignup()} />
                  <input className="field-input" type="password" placeholder="Password (min 8 characters)" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSignup()} />
                  <button className="btn-primary tb" onClick={handleSignup} disabled={loading}>{loading ? 'Creating account…' : 'Create account'}</button>
                </div>
              </div>
