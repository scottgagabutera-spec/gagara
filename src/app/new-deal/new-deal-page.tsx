...
export default function NewDeal() {
  ...
  const canNext = () => {
    ...
    if (step === 2) return !!deal.amount && !!deal.description && !!deal.timeline;
    if (step === 3) {
      if (deal.releaseType === 'milestones') return percentOk && deal.milestones.every(m => m.description.trim());
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge-green  { color: var(--green);    background: var(--green-dim);  border: 0.5px solid rgba(93,204,138,0.2); }
        ...
        .mode-hint {
          ...
          padding: 10px 14px;
          background: var(--indigo-dim);
          border: 0.5px solid rgba(91,79,232,0.15);
          border-radius: var(--r-md);
          font-family: 'DM Mono', monospace;
          font-size: 10px; color: var(--indigo-l); letter-spacing: 0.06em;
        }
        ...
        .rt-desc {
          font-family: 'DM Sans', sans-serif;
          font-size: 10px; color: var(--text-faint); line-height: 1.4;
        }
        ...
        .sb-qr:hover { background: var(--surface3); }
        ...
        .share-divider-text {
          font-family: 'DM Mono', monospace;
          font-size: 9px; color: var(--text-faint); letter-spacing: 0.1em;
        }
        ...
        .btn-next.final:hover { background: #4db87a; }
        ...
        .step-dot {
          ...
          background: var(--border-md); transition: all 0.2s;
        }
        ...
        @media (max-width: 640px) {
          ...
          .milestone-row { grid-template-columns: 1fr 72px 36px; }
      ...
      <div className="shell">
        ...
        <main className="content">
          ...
          {step === 0 && (
            <>
              ...
              <div className="choice-grid">
                <button
                  className={`choice-card ${deal.role === 'payer' ? 'selected' : ''}`}
                  onClick={() => update({role:'payer'})}
                >
                  <div className="choice-icon">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/><line x1="12" y1="12" x2="12" y2="16"/><line x1="10" y1="14" x2="14" y2="14"/></svg>
                  </div>
                  <div className="choice-body">
                    <div className="choice-title">I am paying</div>
                    <div className="choice-desc">You lock funds into the vault. The receiver sees them. Funds release when both of you confirm.</div>
                  </div>
                  <span className="choice-badge badge-indigo">Payer</span>
                </button>
                <button
                  className={`choice-card ${deal.role === 'receiver' ? 'selected' : ''}`}
                  onClick={() => update({role:'receiver'})}
                >
                  <div className="choice-icon">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
                  </div>
                  <div className="choice-body">
                  ...
                  <span className="choice-badge badge-green">Receiver</span>
                </button>
              </div>
            </>
          )}
          ...
          {step === 1 && (
            <>
              <div className="step-header">
                ...
                <div className="step-desc">Each mode has different amount limits and features. This is set once for this deal.</div>
              </div>
              <div className="choice-grid">
                {[
                  { key:'personal', title:'Personal', desc:'Freelancers, small jobs, money between friends. Simple and quick.', range:'$1 – $2,000', badge:'badge-indigo' },
                  { key:'business', title:'Business', desc:'Service contracts, supplier deals, project milestones. Structured and traceable.', range:'$200 – $50,000', badge:'badge-indigo' },
                  { key:'enterprise', title:'Enterprise', desc:'Large contracts, multiple parties, API-driven. Built for deals that demand precision.', range:'$10,000+', badge:'badge-gold' },
                ].map(m => (
                  <button
                    key={m.key}
                    className={`choice-card ${deal.mode === m.key ? 'selected' : ''}`}
                    onClick={() => update({mode: m.key as Mode})}
                  >
                    <div className="choice-body">
            ...
            <>
              ...
              <div className="fields">
                <div className="field">
                  ...
                  <div className="amount-row">
                    <select className="currency-sel" value={deal.currency} onChange={e => update({currency:e.target.value})}>
                      ...
                      <option>KES</option><option>PHP</option><option>NGN</option>
                    </select>
                    <input
                  ...
                  {deal.mode && (
                    <div className="mode-hint">
                      ...
                      {deal.mode.charAt(0).toUpperCase()+deal.mode.slice(1)} mode limit: {modeLimit}
                    </div>
                  )}
                </div>
                <div className="field">
                  ...
                  <textarea
                    ...
                    placeholder="Describe the work, goods, or agreement clearly. This appears in the deal terms both parties see."
                ...
                <div className="field">
                  ...
                  />
                  <div className="field-hint">Both parties can propose timeline changes later. Any change requires mutual confirmation.</div>
                </div>
              </div>
            </>
          )}
          ...
          {step === 3 && (
            <>
              <div className="step-header">
                ...
                <div className="step-desc">Both parties must confirm all conditions are met before any funds are released. Nothing moves without mutual agreement.</div>
              </div>

              {/* Release type */}
              <div className="release-tabs">
                {[
                  { key:'single',     title:'Single release',   desc:'Full amount releases at once on completion.' },
                  { key:'milestones', title:'By milestone',     desc:'Split across multiple stages by percentage.' },
                  { key:'scheduled',  title:'Scheduled',        desc:'Monthly or fixed date tranches.' },
                ].map(r => (
                  <button
                    key={r.key}
                    className={`release-tab ${deal.releaseType === r.key ? 'active' : ''}`}
                    onClick={() => update({releaseType: r.key as Release})}
                  >
                ...
                <>
                  ...
                  <div className="milestones">
                      ...
                      <div key={m.id} className="milestone-row">
                        ...
                        <div style={{position:'relative'}}>
                          <input
                            ...
                            onChange={e => {
                              const ms = [...deal.milestones];
                              ms[i] = {...ms[i], percent: Number(e.target.value)};
                              update({milestones: ms});
                            }}
                          />
                        ...
                        {deal.milestones.length > 2 && (
                          <button
                            className="icon-btn-sm remove"
                            onClick={() => update({milestones: deal.milestones.filter((_,j) => j !== i)})}
                    ...
                    <div className="total-row">
                      ...
                      <span className={totalPercent === 100 ? 'percent-ok' : 'percent-err'}>
                        {totalPercent}% {totalPercent === 100 ? '— correct' : `— must equal 100%`}
                      </span>
                    </div>
              ...
              {/* Scheduled */}
              {deal.releaseType === 'scheduled' && (
                <div className="fields">
                  <div className="field">
                    <div className="field-label">Payment schedule</div>
                    <textarea
                      className="field-input"
                      placeholder="e.g. $500 on the 1st of each month for 12 months. Final payment of $500 on completion."
                      rows={3}
                      value={deal.description.includes('schedule:') ? deal.description.split('schedule:')[1] : ''}
                      onChange={e => update({description: deal.description.split('schedule:')[0] + 'schedule:' + e.target.value})}
                    />
                    <div className="field-hint">Funds remain locked in the vault and release automatically on each scheduled date — only after both parties confirm.</div>
                  </div>
                </div>
              )}
              ...
              <div style={{marginTop:'8px'}}>
                <div className="field-label" style={{marginBottom:'8px'}}>
                  Release conditions
                  <span style={{marginLeft:'8px',fontFamily:'DM Sans',fontSize:'10px',color:'var(--text-faint)',textTransform:'none',letterSpacing:'normal'}}>— both parties must confirm each one</span>
                </div>
                  ...
                  {deal.conditions.map((c, i) => (
                    <div key={c.id} className="condition-row">
                      <textarea
                        className="field-input"
                        rows={2}
                        placeholder={`Condition ${i+1} — e.g. "Final files delivered in agreed formats" or "Goods received in good condition"`}
                        value={c.text}
                        onChange={e => {
                          const cs = [...deal.conditions];
                          cs[i] = {...cs[i], text: e.target.value};
                          update({conditions: cs});
                        }}
                      />
                      {deal.conditions.length > 1 && (
                        <button
                          className="icon-btn-sm remove"
                          onClick={() => update({conditions: deal.conditions.filter((_,j) => j !== i)})}
              ...
              <div className="legal-note">
                ...
                <div className="legal-text">
                  <strong>If there is a dispute</strong> — Gagara generates a full PDF record of all agreed conditions, every confirmation, and the complete audit trail. This document is timestamped, signed, and court-ready. Both parties can download it at any time.
                </div>
              </div>
            </>
          )}
          ...
          {step === 4 && (
            <>
              <div className="step-header">
                ...
                <div className="step-desc">Check everything before generating the Deal Code. The other party will see exactly this.</div>
              </div>
              ...
              <div className="review-card">
                ...
                <div className="review-section">
                  ...
                  <div className="review-row"><span className="review-key">Deadline</span><span className="review-val">{deal.timeline}</span></div>
                  <div className="review-row"><span className="review-key">Release type</span><span className="review-val">{deal.releaseType === 'single' ? 'Single release' : deal.releaseType === 'milestones' ? `${deal.milestones.length} milestones` : 'Scheduled'}</span></div>
                </div>
                <div className="review-section">
                  <div className="review-label">Release conditions — both parties must confirm</div>
              ...
              <div className="confirm-notice">
                ...
                <span>The other party will review and confirm all terms before any funds move. Nothing is locked until both sides agree.</span>
              </div>
            </>
          )}
          ...
          {step === 5 && (
            <>
              <div className="step-header">
                ...
                <div className="step-desc">Share this code with the other party. They enter it in Gagara to review and accept the deal terms.</div>
              </div>
              ...
              <div className="deal-code-box">
                ...
                <div className="share-actions">
                  <button className={`share-btn sb-copy ${copied ? 'done' : ''}`} onClick={copyCode}>
                    {copied ? (
                      <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>Copied</>
                    ) : (
                      <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy code</>
                    )}
                  </button>
                  <button className="share-btn sb-whatsapp">
                  ...
                  <button className="share-btn sb-qr">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><path d="M14 14h.01M14 17h.01M17 14h.01M17 17h3M20 14v.01"/></svg>
                    Show QR code
                  </button>
                </div>
              </div>
              ...
              <div className="share-divider">
                ...
                <div className="share-divider-text">If there is a dispute</div>
                <div className="share-divider-line" />
              </div>
              ...
              <div className="pdf-note">
                ...
                <div className="pdf-note-text">
                  <strong>Gagara generates a court-ready PDF</strong> of the full deal — all agreed conditions, every confirmation, and the complete timestamped audit trail. Both parties can download it at any time from the deal page. It is admissible as evidence in legal proceedings.
                </div>
              </div>
            </>
          )}
          ...
          <div className="step-footer">
            ...
            {step < 5 && (
              <button
                className={`btn-next ${step === 4 ? 'final' : ''}`}
                onClick={next}
                disabled={!canNext()}
              >
                {step === 4 ? 'Generate Deal Code' : 'Continue'}
