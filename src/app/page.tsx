    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
          ...
          /* THE KEY: body itself never scrolls horizontally */
          overflow-x: hidden;
          max-width: 100%;
        }
        ...
        /* page-max: hard contain — nothing inside can scroll past the viewport */
        ...
        .vault-wrap {
          padding: 0 48px 96px;
          /* Hard wall: nothing inside can bleed horizontally */
          overflow: hidden;
          width: 100%;
        }
        ...
        .vault-card {
          ...
          /* Clip children — nothing escapes */
          overflow: hidden;
          background: var(--surface);
          width: 100%;
        }

        /* minmax(0, 1fr) is the fix: forces grid children to shrink within the track */
        .vault-cols {
          display: grid;
        ...
        .party-col {
          ...
          /* Prevent text/elements from blowing out column width */
          overflow: hidden;
          min-width: 0;
        }
        ...
        .p-eye {
          ...
          padding: 9px 13px; background: var(--indigo-dim);
          border: 0.5px solid rgba(84,72,228,0.2); border-radius: var(--r-sm);
          font-family: 'IBM Plex Mono', monospace; font-size: 9px; letter-spacing: 0.07em; color: var(--indigo-l);
          /* Never wrap — prevents layout shift */
        ...
        /* Vault center — strictly bounded */
        .vault-mid {
          display: flex; flex-direction: column; align-items: center; justify-content: center;
          padding: 32px 16px; gap: 14px; position: relative;
        ...
        /* VAULT FIGURE: absolute fixed size — no animation, no resize, ever */
        .vault-fig {
          position: relative;
        ...
        @media (max-width: 1024px) {
          ...
          /* Tight padding so vault doesn't approach screen edges */
          .vault-wrap { padding: 0 16px 80px; }

          /* Stack to single column — no horizontal overflow possible */
          ...
          /* Vault center: compact horizontal row on mobile */
          .vault-mid {
          ...
          /* Wires don't work in stacked layout */
          .wire-svg { display: none; }

          /* Vault is smaller on mobile but still fixed — never grows */
          .vault-fig {
          ...
          /* Price tag repositioned for smaller vault */
          ...
          <div className="vault-card">
            <div className="vault-cols">
              ...
              <div className="vault-mid">
                ...
                <div className="vault-note">Held by Gagara<br />Neither party acts alone</div>
              </div>
              ...
              <div className="party-col recv">
                ...
                <div>
                  <div className="p-amount-tag">{released ? 'Received' : funded ? 'Waiting for release' : 'Not funded yet'}</div>
      ...
      <div className="page-max">
        <section className="section" id="how">
          ...
          <p className="sec-desc">Five stages, in order, every time. Nothing is skipped. Every action is recorded and visible to both parties.</p>
          <div className="steps-grid">
            {[
              { n:'01', title:'Both parties connect',    desc:'One person creates the deal and shares a code. The other enters it. The deal is live.', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg> },
              { n:'02', title:'Money goes into the vault', desc:'The payer deposits funds. Both sides can see the balance. Neither can touch it alone.', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg> },
              { n:'03', title:'Work gets done',           desc:'The job proceeds. Milestones are tracked. Every update is timestamped and recorded.', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg> },
              { n:'04', title:'Both sides confirm',       desc:'When conditions are met, both parties confirm. One confirmation is never enough.', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> },
              { n:'05', title:'Payment is released',      desc:'Funds transfer instantly via Interledger. The deal record stays permanently on file.', icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"><polyline points="5 12 12 5 19 12"/><line x1="12" y1="5" x2="12" y2="19"/></svg> },
            ].map((s, i) => {
      ...
      <div className="page-max">
        <section className="section" id="modes">
          ...
          <p className="sec-desc">Your account type is set once when you sign up. The deal mode is chosen each time you create a new agreement.</p>
          <div className="modes-grid">
            {[
              { n:'01', gold:false, icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--indigo-l)" strokeWidth="1.3" strokeLinecap="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>, title:'Personal',   desc:'Freelancers, small jobs, money between friends. Simple terms and quick to set up.',                           range:'$1 to $2,000',       features:['Single release condition','Deal code expires in 48 hours','Instant mutual release','Full activity record'] },
              { n:'02', gold:false, icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--indigo-l)" strokeWidth="1.3" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>, title:'Business',   desc:'Service contracts, supplier payments, project milestones. Structured and fully traceable.',                  range:'$200 to $50,000',    features:['Multiple milestone releases','Edit timelines with full history','Invoice generation','Priority dispute support'] },
              { n:'03', gold:true,  icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--gold)"     strokeWidth="1.3" strokeLinecap="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/></svg>,  title:'Enterprise', desc:'Large contracts, multiple parties, API integration. Built for deals that demand precision.',              range:'$10,000 and above',  features:['Multi-party deals','API access','Compliance documentation','Dedicated mediation team'] },
            ].map((m, i) => (
      ...
      <div className="page-max">
        <section className="section" id="payouts">
          ...
          <p className="sec-desc">The receiver picks how they want to be paid. Gagara shows the estimated arrival time before anything is confirmed. Powered by Interledger Protocol — it works across any currency and any network.</p>
          <div className="payout-table">
            <div className="pt-head"><span>Payment method</span><span>How fast</span></div>
            {[
        ...
        <div className="closing">
          <div className="closing-box">
            <div className="cb-inner">
              ...
              <h2 className="cb-h">Work should be simple.<br /><em>Get paid when the job is done.</em></h2>
              <p className="cb-p">Lock funds. Confirm the work. Release with certainty. For freelancers in Manila, contractors in Lagos, suppliers in London and everyone in between.</p>
